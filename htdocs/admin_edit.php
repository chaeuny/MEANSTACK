...
// admin 페이지가 뜨면 관리자인건 인증이 된 상태임. 

  $edit_emp_no = $_POST[ 'edit_emp_no' ];
  $jb_conn = mysqli_connect( 'localhost', 'root', '', 'hsh' );
  $jb_sql_edit = "SELECT * FROM user WHERE num = $edit_emp_no;";
    ...
    <div class="container"> 
      ...
      <div class="col-lg-4">
        <div class="jumbotron" style="padding-top: 20px;">
          <form name="join"  method="post" action="admin_edit_ok.php">
            <h3 style="text-align: center"> Admin_Edit <br> <P>user_no. <?php echo $jb_row[ 'num' ]; ?></P></h3>
            <div class="col-lg-4"></div>
            <div class="form-group">
               <input type="hidden" class="form-control"  name="num" value="<?php echo $jb_row[ 'num' ]; ?>" />
              <input type="text" class="form-control" placeholder="아이디" name="id" id="id" maxlength="15"  value="<?php echo 'id = '.$jb_row[ 'id' ]; ?>" disabled> <br>
              <input type="text" class="form-control" placeholder="아이디" name="role" id="role" maxlength="15"    value="<?php echo '권한 = ' . $jb_row[ 'role' ]; ?>" disabled>
            ...
            <div class="form-group">
              <input type="text" class="form-control" placeholder="이름" name="name" id="name" maxlength="20" value="<?php echo $jb_row[ 'name' ]; ?>">
            </div>
            
            <div class="form-group" style="text-align: center">
              <div class="btn-group" data-toggle="buttons" >
                <label class="btn btn-primary active">
                  <input type="radio" name="gender" id="gender1" autocomplete="off" value="남자" <?php echo $jb_row["gender"]=="남자" ? "checked" : "" ?> >남자
                </label>
                <label class="btn btn-primary">
                  <input type="radio" name="gender" id="gender2" autocomplete="off" value="여자" <?php echo $jb_row["gender"]=="여자" ? "checked" : "" ?> >여자
                </label>
              </div>
            </div>

            <div class="form-group">
              <input type="tel" class="form-control" placeholder="전화번호" name="phone" id="phone" value="<?php echo $jb_row[ 'phone' ]; ?>" maxlength="20">
            </div>  
            <div class="col-lg-4"></div>
            <div class="form-group">
              <input type="email" class="form-control" placeholder="이메일" name="email" id="email" value="<?php echo $jb_row[ 'email' ]; ?>" maxlength="80">
            </div>
          
            <span class="btn btn-primary form-control" onclick="check_input()">수정하기 </span>&nbsp;
            <span class="btn btn-primary form-control" onclick="reset_form()">초기화</span>&nbsp;
           <a href="admin.php"><span class="btn btn-primary form-control" >뒤로가기</span></a>  
          </div>
    </div>
          </form>
        </div>
