<?php   
include "/0531/db.php"; 
include "../db.php"; 
?>
<!DOCTYPE html>
<head>
	<meta charset="utf-8" />
	<title>회원가입 및 로그인 사이트</title>
<link rel="stylesheet" type="text/css" href="/0531/css/common.css" />
</head>
<body>
	<div id="login_box" style="margin-top:30px; ">
		<h1>로그인</h1>							
			<form method="post" action="/0531/member/login_ok.php">
				<table align="center" border="0" cellspacing="0" width="300">
        			<tr>
            			<td width="120" colspan="1"> 
                		<input type="text" name="userid" class="inph">
            		</td>
            		<td rowspan="2" align="center" width="100" > 
                		<button type="submit" id="btn" >로그인</button>
            		</td>
        		</tr>
        		<tr>
            		<td width="120" colspan="1"> 
               		<input type="password" name="userpw" class="inph">
            	</td>
        	</tr>
        	<tr>
           		<td colspan="3" align="center" class="mem"> 
              	<a href="/member/member.php">회원가입 하시겠습니까?</a>
                <a href="/member/member_find.php">계정찾기</a>
           </td>
        </tr>
    </table>
  </form>
</div>
</body>
</html>